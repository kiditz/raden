...
public class EditMask {
	...
	/**
	 * @param string
	 *            Sets the text string in the receiver
	 */
	...
	/**
	 * setRawText takes raw text as a parameter but formats it before setting
	 * the text in the Text control.
	 * 
	 * @param string
	...
	/**
	 * @return true if the field is complete according to the mask; false
	 *         otherwise
	 */
	 ...
	 * different character than any character that is allowed as input anywhere
	 * in the edit mask. For example, if the edit mask permits spaces to be used
	 * as input anywhere, the placeholder character must be something other than
	 * a space character.
	 * <p>
	 ...
	 * allowed as input anywhere in the edit mask. For example, if the edit mask
	 * permits spaces to be used as input anywhere, the placeholder character
	 * must be something other than a space character.
	 * <p>
	 * The space character is the default placeholder character.
	 * 
	 * @param placeholder
	 *            The character to use as a placeholder
	 */
	...
	private VerifyListener verifyListener = new VerifyListener() {
		public void verifyText(VerifyEvent e) {
			...
			// any new characters
			if (editMaskParser.isComplete() && // should eventually be .isFull()
												// to account for optional
												// characters
			...
			if (!updating) {
				...
				// If the machine is loaded down (ie: spyware, malware), we
				// might
	...
	private Runnable updateTextField = new Runnable() {
		...
		private void updateSelectionPosition(String newRawText) {
			...
			if (isInsertingNewCharacter(newRawText) || replacedSelectedText) {
				// Find the position after where the new character was actually
				// inserted
			...
			if (!newRawText.equals(oldRawText)) { // yep
				...
				// If the user hits <end>, bounce them back to the end of their
				// actual input
				...
				// Either we backspaced over a literal or we typed an illegal
				// character
				if (selection > oldSelection) { // typed an illegal character;
												// backup
					text.setSelection(new Point(selection - 1, selection - 1));
				} else { // backspaced over a literal; don't interfere with
							// selection position
					text.setSelection(new Point(selection, selection));
				}
			}
